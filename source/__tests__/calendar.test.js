...
describe('Basic user flow for Website', () => {
    ...
    */
    it('Check if the correct colors are given to each cell', async () => {
        ...
        // Get daily budget and calendar table values
        let dailyBudget = localStorageBudget / 31;
        var table = await page.$("#calendar-body");
        let tableHTML = await table.getProperty('innerHTML');
        let tableHTMLVal = await tableHTML.jsonValue();
        // console.log(tableHTMLVal);
        for (var i = 0; i < userExpenseTable.length; i++) {
            // Get amount spent per day
            let element = userExpenseTable.at(i);
            let dailySpent = element['cost'];
            let date = element['date'].charAt(9);
            // Get correct color
            let color = getColor(dailySpent, dailyBudget);
            // Look for cell in table
            let string = `<td class="${color}">${date}</td>`;
            console.log(string);
            if (!tableHTMLVal.includes(string)) {
                passed = false;
            }
        }
    ...
    /* Function for getting the correct color for each sample data in the 3rd test */
    function getColor(dailySpent, dailyBudget) {
        /* dailySpent/dailyBudget > 1.2 should be dark red */
        if (dailySpent > dailyBudget * 1.2) {
            return 'bg-info-dark-red';
        }
        /* dailySpent/dailyBudget > 1 should be light red */
        else if (dailySpent > dailyBudget) {
            return 'bg-info-light-red';
        }
        /* dailySpent/dailyBudget > 0.8 should be light green */
        else if (dailySpent > dailyBudget * 0.8) {
            return 'bg-info-light-green';
        }
        /* dailySpent/dailyBudget > 0 should be dark green */
        else if (dailySpent > 0) {
            return 'bg-info-dark-green';
        }
    }
